...
public enum OrderErrorCodeEnum implements BaseErrorCodeEnum {
     ...
     * 逆向订单错误码106开头
     */
    RETURN_GOODS_REQUEST_IS_NULL("106051", "手动退货入参不能为空"),
    AFTER_SALE_TIME_IS_NULL("106052", "申请售后时间不能为空"),
    ORDER_CURRENT_TYPE_IS_NULL("106053", "当前订单状态不能为空"),
    SKU_IS_NULL("106054", "sku列表不能为空"),
    RETURN_GOODS_NUM_IS_NULL("106055", "退货数量不能为空"),
    RETURN_GOODS_CODE_IS_NULL("106056", "退货原因选项不能为空"),
    AFTER_SALE_TYPE_IS_NULL("106057", "售后类型不能为空"),
    ORDER_STATUS_CHANGED("106058", "当前订单状态已变更，请重新刷新"),
    ORDER_STATUS_CANCELED("106058", "当前订单已取消"),
    ORDER_STATUS_IS_NULL("106059", "当前订单状态不能为空"),
    ORDER_PAY_TRADE_NO("106060", "当前订单已产生支付流水号，不能取消"),
    CANCEL_REQUEST_IS_NULL("106061", "取消订单入参不能为空"),
    CANCEL_TYPE_IS_NULL("106062", "取消订单类型不能为空"),
    CANCEL_ORDER_ID_IS_NULL("106063", "取消订单ID不能为空"),
    CANCEL_USER_ID_IS_NULL("106064", "取消订单用户ID不能为空"),
    CANCEL_ORDER_REPEAT("106065", "取消订单重复"),
    CANCEL_ORDER_FULFILL_ERROR("106066", "取消履约失败"),
    PROCESS_REFUND_REPEAT("106067", "处理退款重复"),
    CALCULATING_REFUND_AMOUNT_FAILED("106071", "取消订单用户ID不能为空"),
    PROCESS_REFUND_FAILED("106072", "退款前准备工作失败"),
    SEND_MQ_FAILED("106073", "发送MQ消息失败"),
    CONSUME_MQ_FAILED("106074", "消费MQ消息失败"),
    ORDER_REFUND_AMOUNT_FAILED("106075", "调用支付退款接口失败"),
    PROCESS_PAY_REFUND_CALLBACK_REPEAT("106076", "处理支付退款回调重复"),
    PROCESS_PAY_REFUND_CALLBACK_FAILED("106077", "处理支付退款回调失败"),
    PROCESS_PAY_REFUND_CALLBACK_BATCH_NO_IS_NULL("106078", "处理支付退款回调批次号不能为空"),
    PROCESS_PAY_REFUND_CALLBACK_STATUS_NO_IS_NUL("106079", "处理支付退款回调退款状态不能为空"),
    PROCESS_PAY_REFUND_CALLBACK_FEE_NO_IS_NUL("106080", "处理支付退款回调退款金额不能为空"),
    PROCESS_PAY_REFUND_CALLBACK_TOTAL_FEE_NO_IS_NUL("106081", "处理支付退款回调退款总额不能为空"),
    PROCESS_PAY_REFUND_CALLBACK_SIGN_NO_IS_NUL("106082", "处理支付退款回调签名不能为空"),
    PROCESS_PAY_REFUND_CALLBACK_TRADE_NO_IS_NUL("106083", "处理支付退款回调交易号不能为空"),
    PROCESS_AFTER_SALE_RETURN_GOODS("106084", "处理售后退款重复"),
    PROCESS_PAY_REFUND_CALLBACK_AFTER_SALE_ID_IS_NULL("106085", "处理支付退款回调售后订单号不能为空"),
    PROCESS_PAY_REFUND_CALLBACK_AFTER_SALE_REFUND_TIME_IS_NULL("106086", "处理支付退款回调售后退款时间不能为空"),
    REPEAT_CALLBACK("106087", "重复的退款回调"),
    REPEAT_CALL("106088", "当前接口被重复调用"),
    REFUND_MONEY_REPEAT("106089", "执行退款操作重复"),
    ORDER_CANNOT_REPEAT_OPERATE("106090", "当前订单不能重复操作"),
    PROCESS_APPLY_AFTER_SALE_CANNOT_REPEAT("106091", "不能重复发起售后"),
    CUSTOMER_AUDIT_CANNOT_REPEAT("106092", "不能重复处理客服审核信息"),
    AFTER_SALE_ITEM_CANNOT_NULL("106093", "售后商品信息不能为空"),
    CUSTOMER_AUDIT_RESULT_IS_NULL("106094", "处理客服审核结果不能为空"),
    INSERT_ORDER_TRANSACTION_MQ_ERROR("106095", "生成订单失败"),
    CURRENT_ORDER_STATUS_CANNOT_CANCEL("106096", "当前订单状态不允许取消"),
    SEND_TRANSACTION_MQ_FAILED("106097", "发送MQ事务消息失败"),
    UPDATE_ORDER_INFO_AND_SAVE_ORDER_LOG_FAILED("106098", "更新订单信息和保存订单操作日志失败"),
    CANCEL_ORDER_PROCESS_FAILED("106099", "取消订单过程执行失败"),
    REFUND_MONEY_RELEASE_COUPON_FAILED("106100", "实际退款释放优惠券失败"),
    SEND_AUDIT_PASS_RELEASE_ASSETS_FAILED("106101", "发送审核通过释放资产topic失败"),
    SEND_AFTER_SALE_CUSTOMER_AUDIT_MQ_FAILED("106102", "发送客服审核售后事务消息失败"),
    ORDER_CANNOT_OPERATED_IN_MULTIPLE_PLACES("106103", "当前订单不能在多处同时操作"),
    AFTER_SALE_ORDER_STATUS_ERROR("106104", "售后单状态错误"),
    AFTER_SALE_QUANTITY_IS_ERROR("106105", "售后数量错误"),
    AFTER_SALE_COUPON_IS_ERROR("106106", "售后查询优惠券失败"),
    AFTER_SALE_TYPE_IS_ERROR("106107", "优惠券售后单无需审核"),
    VIRTUAL_ORDER_CANNOT_AFTER_SALE("106108", "虚拟订单不能售后"),
    PAY_AMOUNT_IS_NULL("106109", "支付金额不能为空"),
    TOTAL_PAY_AMOUNT_ERROR("106110", "总的支付金额错误"),
    PAY_REFUND_CALLBACK_STATUS_FAILED("106111", "支付退款回调状态错误"),
